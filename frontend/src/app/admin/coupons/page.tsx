...
export default function AdminCouponsPage() {
  ...
  const { data: coupons = [], isLoading } = useQuery({
    ...
    queryFn: () => adminApi.coupons.list(token!),
    enabled: !!token,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: CouponValues) =>
      editing ? adminApi.coupons.update(token!, editing._id, values) : adminApi.coupons.create(token!, values),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => adminApi.coupons.remove(token!, id),
    onSuccess: () => {
