...
export default function AdminProductsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => adminApi.products.list(token!, { q: q || undefined, limit: 50 }),
    enabled: !!token,
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => adminApi.products.remove(token!, id),
    onSuccess: () => {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <Button className="rounded-full" render={<Link href="/quan-tri/san-pham/moi" />}>
              ...
              <tr key={p._id} className="hover:bg-muted/30">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <Link href={`/quan-tri/san-pham/${p._id}`} className="font-medium hover:underline">
                      {p.name}
                    </Link>
                  </div>
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex justify-end gap-2">
                    <Button variant="ghost" size="icon-sm" render={<Link href={`/quan-tri/san-pham/${p._id}`} />} aria-label="Sửa">
